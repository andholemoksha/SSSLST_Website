"""Django admin configuration for written reflections."""
...
@admin.register(DhyanaVahiniText)
class DhyanaVahiniTextAdmin(admin.ModelAdmin):
	change_list_template = 'admin/website/dhyanavahinitext/change_list.html'
	...
	def get_urls(self):
		custom_urls = [
			path(
				...
				name='website_dhyanavahinitext_import_csv',
			),
		]
	...
	def import_csv_view(self, request):
		if request.method == 'POST':
			...
			existing = {
				...
				for item in DhyanaVahiniText.objects.filter(year=year)
			}
			...
			self.message_user(
				request,
				f'Imported {len(rows)} reflection(s) for {year}.',
				messages.SUCCESS,
			)
			return HttpResponseRedirect(reverse('admin:website_dhyanavahinitext_changelist'))
	...
	def _render_import(self, request, error='', year=''):
		return render(request, 'admin/website/dhyanavahinitext/import_csv.html', {
